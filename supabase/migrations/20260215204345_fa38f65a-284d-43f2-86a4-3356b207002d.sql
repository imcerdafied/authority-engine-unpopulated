...
-- Server-side function to compute all overview metrics
...
DECLARE
  ...
  v_drivers text[] := '{}';
BEGIN
  -- Active High-Impact: count(decisions where impact_tier=High and status=Active)
  SELECT COUNT(*) INTO v_active_high
  FROM decisions
  WHERE org_id = _org_id AND status = 'Active' AND impact_tier = 'High';
  ...
  -- Overdue slices: active decisions past their slice_due_at
